...
class TestPrCommentCLI:
    ...
    def test_scan_pr_comment_fail_on_info_flips_header(self, tmp_path):
        """`--fail-on info` must flip the header to FAIL on a sub-HIGH finding (#82 edge case).

        The file carries only an INFO-level AI-generated marker. At the default
        HIGH threshold the header is PASS; lowering --fail-on to info promotes
        the INFO finding to blocking, so the header (and the blocking section)
        must reflect FAIL — while scan still exits 0.
        """
        (tmp_path / "app.py").write_text("# Generated by ChatGPT\nprint('hi')\n")

        default = runner.invoke(app, ["scan", "--path", str(tmp_path), "--pr-comment"])
        assert default.exit_code == 0
        assert "— PASS" in default.stdout

        info = runner.invoke(
            app, ["scan", "--path", str(tmp_path), "--pr-comment", "--fail-on", "info"]
        )
        assert info.exit_code == 0
        assert "— FAIL" in info.stdout
        assert "### Blocking Findings" in info.stdout
